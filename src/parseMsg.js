...
function parseMsg($msg) {
    ...
    if ($msg.is('.message_system')) {
        ...
        const size = $msg.find('span:first').text();
        const $download = $msg.find('a[download]'); // 可触发下载
        ...
        $msg[0].click();
        const duration = parseInt($msg.find('.duration').text());
        ...
        const name = $msg.find('.display_name').text();
        const wxid = $msg.find('.signature').text(); // 微信注释掉了
        ...
        var ctn = '';
        let normal = false;
        const $text = $msg.find('.js_message_plain');
        $text.contents().each((i, node) => {
